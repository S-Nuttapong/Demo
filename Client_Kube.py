...
def save_result(server_response, output_image):
    
    response = json.loads(server_response.text)
    predictions = response['predictions'][0]
    client_result_df = pd.DataFrame(predictions)
    client_result_df.drop(['num_detections', 'raw_detection_boxes', 'raw_detection_scores', 'detection_multiclass_scores'], axis=1, inplace=True)
    client_result_df[client_result_df['detection_scores'] > 0.8].to_json(output_image)
    ...
    with open(output_image, 'w') as f:
        json.dump(json_obj, f, indent=4, separators=(',', ':'), sort_keys='True') #dump for write, dumps for read
        f.write('\n')
  

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Performs call to the tensorflow-serving REST API.')
    parser.add_argument('--server_url', dest='server_url', type=str,
                        default='http://35.236.173.101:8501/v1/models/faster_rcnn_traffy:predict',
                        help='URL of the tensorflow-serving accepting API call. '
                             'e.g. http://localhost:8501/v1/models/omr_500:predict')
#     parser.add_argument('--image_fname', dest='image_path', type=str,
...
#                              'to a unique number.')
    ...
    img_root_dir = os.path.join(os.curdir, 'Test/road')
    result_root_dir = os.path.join(os.curdir, 'Test_Result')
    fnames = os.listdir(img_root_dir)
    print(fnames[1:])
    fname = input('Chose Image File: ')
    image_path = os.path.join(img_root_dir, fname)
    
    output_json = os.path.join(result_root_dir, fname.split('.')[0] + '.json')
    output_image = os.path.join(result_root_dir, fname)
    save_output_image = args.save_output_image
    path_to_labels = os.path.join(os.curdir, 'data/faster_rcnn_road/label_map.pbtxt')
